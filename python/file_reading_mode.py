...
def intesityPlot(data, N_ROWS):
    ...
    for i in range(N_ROWS):
        ...
        for j in range(len(values)):
            values[j] -= max_value
        values = values * -1
        colors = genColorGradient("blue", "red")
        color_groups = values.max() / 10
        for j in range(len(values)):
            if values[j] <= color_groups:
                temp = bytearray.fromhex(colors[0].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif color_groups < values[j] <= 2 * color_groups:
                #string = colors[1].get_hex_l()
                temp = bytearray.fromhex(colors[1].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 2 * color_groups < values[j] <= 3 * color_groups:
                #print(colors[2].get_hex_l()[1:])
                temp = bytearray.fromhex(colors[2].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 3 * color_groups < values[j] <= 4 * color_groups:
                temp = bytearray.fromhex(colors[3].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 4 * color_groups < values[j] <= 5 * color_groups:
                temp = bytearray.fromhex(colors[4].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 5 * color_groups < values[j] <= 6 * color_groups:
                temp = bytearray.fromhex(colors[5].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 6 * color_groups < values[j] <= 7 * color_groups:
                temp = bytearray.fromhex(colors[6].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 7 * color_groups < values[j] <= 8 * color_groups:
                temp = bytearray.fromhex(colors[7].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            elif 8 * color_groups < values[j] <= 9 * color_groups:
                temp = bytearray.fromhex(colors[8].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]

            else:
                temp = bytearray.fromhex(colors[9].get_hex_l()[1:])
                for index in range(len(temp)):
                    leds[j*3+index] = temp[index]
